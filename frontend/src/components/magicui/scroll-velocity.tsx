...
export function ScrollVelocityContainer({
  ...
  const velocityFactor = useTransform(smoothVelocity, (v) => {
    const sign = v < 0 ? -1 : 1;
    const magnitude = Math.min(25, (Math.abs(v) / 1000) * 25);
    return sign * magnitude;
  });

  return (
    <ScrollVelocityContext.Provider value={velocityFactor}>
      <div className={cn("relative w-screen -ml-4", className)} {...props}>
        {children}
      </div>
...
function ScrollVelocityRowImpl({
  ...
  return (
    <div
      ref={containerRef}
      className={cn("w-screen whitespace-nowrap -ml-4", className)}
      {...props}
    >
      <motion.div
